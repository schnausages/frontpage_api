...
def frontpage():
    ...
    ordered_ref = article_ref.order_by(
        u'added', direction=firestore.Query.DESCENDING)
...
@app.route('/chargeForCookie', methods=['GET', 'POST'])
def charge():
    if request.method == 'POST':
        return 200
...
def send_trending_news():
    requests_cache.remove_expired_responses()
    trending_response = req.get(
        'http://newsapi.org/v2/top-headlines?country=us&pageSize=12&apiKey=8e5ad43b589749e8a7c76bbb04ec8ed6')
...
def send_gaming_news():
    requests_cache.remove_expired_responses()
    gaming_response = req.get(
        'http://newsapi.org/v2/everything?q=video games OR gaming OR esports&pageSize=10&sortBy=popularity&'f'from={date}&apiKey=8e5ad43b589749e8a7c76bbb04ec8ed6')
...
def send_socialmedia_news():
    requests_cache.remove_expired_responses()
    socialmedia_response = req.get(
        'http://newsapi.org/v2/everything?q=instagram OR tiktok OR twitter OR snapchat OR youtube OR facebook OR social-media&'f'from={date}&pageSize=10&apiKey=8e5ad43b589749e8a7c76bbb04ec8ed6')
...
def send_science_news():
    requests_cache.remove_expired_responses()
    science_response = req.get(
        'http://newsapi.org/v2/top-headlines?category=science&country=us&pageSize=6&apiKey=8e5ad43b589749e8a7c76bbb04ec8ed6')
...
def send_politics_news():
    requests_cache.remove_expired_responses()
    politics_response = req.get(
        'http://newsapi.org/v2/everything?q=politics OR democrats OR president trump OR trump OR pelosi OR white house OR republicans OR supreme-court &sortBy=relevancy&'f'from={date}&pageSize=10&apiKey=8e5ad43b589749e8a7c76bbb04ec8ed6')
...
def send_popculture_news():
    requests_cache.remove_expired_responses()
    popculture_response = req.get(
        'http://newsapi.org/v2/top-headlines?category=entertainment&country=us&pageSize=10&apiKey=8e5ad43b589749e8a7c76bbb04ec8ed6')
